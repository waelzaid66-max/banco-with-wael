...
test("sale engine never serializes rental_term even if criteria stale", () => {
  const params = buildSearchParams({
    ...DEFAULT_CRITERIA,
    category: "real_estate",
    engineKey: "sale",
  ...
  assert.equal(params.offer_type, "sale");
  assert.equal(params.rental_term, undefined);
});

test("propertyType composes with rent offer without flipping offer_type", () => {
  const params = buildSearchParams({
    ...DEFAULT_CRITERIA,
    category: "real_estate",
    engineKey: "rent",
  ...
  assert.equal(params.offer_type, "rent");
  assert.equal(params.property_type, "office");
});
