/**
 * B-CORE Industrial Hub — premium black header (visual shell).
 *
 * Stay/B-PROPERTIES bands (A–D): back/save · B-CORE wordmark · search+filter
 * pill · large industrial type tabs + market.
 * Presentational only — parent (`SectionSearchApp`) owns criteria and sheets.
 * Materials-only; do not mount from Cars / Stay / RE / Factories.
 */
...
/** Large industrial icons — registry names only (Android/Expo-safe). */
...
export function MaterialsHomeHeader({
  ...
  onOpenMarket,
  onCycleSort,
}: MaterialsHomeHeaderProps) {
  const insets = useSafeAreaInsets();
  const { t, isRTL } = useI18n();
  // Owner rule: never invent a fake 67px web pad.
  ...
  return (
    <View style={[styles.root, { paddingTop: topPad - 1 }]} testID="materials-core-header">
      ...
      {/* Band B — B-CORE identity (balanced, not half-screen) */}
      ...
      {/* Band C — search pill; Filters live inside (compressed, not erased) */}
      {searchOpen ? (
      ...
      {/* Band D — market + LARGE industrial type icons */}
      <ScrollView
        horizontal
